...
public class TransfereService
{
    ...
    public TransfereResponse payerTransfereGab(String reference, String pinTransfere)
    {
        ...
        if(transfere.getDateExpiration().after(new Date()) && transfere.getStatus() == StatusTransfere.A_SERVIR && pinTransfere.equalsIgnoreCase(transfere.getCodePinTransfere()))
        {
            ...
            Client clientBeneficiaire = restTemplate.getForObject("http://client-service/api/client/get-client-cin/"+transfere.getReferenceClientBeneficiaire(), Client.class);
    ...
    public void extournerTransfere(String reference)
    {
        ...
        Agent agent = restTemplate.getForObject("http://agent-service/api/agent/ref/"+transfere.getReferenceAgent(), Agent.class);
